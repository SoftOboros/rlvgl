...
| 2026-07-12 | §6 conformance amendment. `DimResolver` now builds a small precedence-aware Rust integer-expression tree and the sibling box-model solver composes through the same tree. Rendering inserts parentheses only where QML grouping or right-side associativity requires them, preserving integer evaluation for nested `+`, `-`, `*`, and `/`. The tree removes only semantics-preserving neutral operations (`+ 0`, `- 0`, `* 1`, `/ 1`); legacy non-sibling bounds paths also omit zero offsets. Covered by focused grouping, neutral-offset, and generated-source tests. Version bump shared with the QT-03b conformance amendment: `QT_EMIT_VERSION_RLVGL` `22 → 23`. |
