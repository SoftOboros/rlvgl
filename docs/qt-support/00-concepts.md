...
| `QT-10`   | Strict-mode acceptance + release tag        | [`10-release.md`](./10-release.md) — ratified; strict-mode-2 shipped (auto-mode half); release tag is user-driven |
...
| 2026-07-12 | Qt emitted-Rust conformance amendment shipped across QT-03b/QT-03c/QT-04f. The rlvgl emitter now uses precedence-aware integer expressions, removes neutral bounds operations, returns leaf nodes directly, routes binding accumulation through a private sink, exposes a documented `BuiltScreen` tuple alias, emits `ClickArea` mutability only when required, and converts generated Rust symbols to deterministic snake case while retaining original QML tags and lookup metadata. `QT_EMIT_VERSION_RLVGL` `22 → 23`. QT-10 advances to strict-mode-2 and reconciles its 29-chapter/version snapshot. |
