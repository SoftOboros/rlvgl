...
| 2026-07-12 | §3/§8 conformance amendment. Generated `build_screen` signatures return a documented `BuiltScreen` tuple alias; private `BindingSink` values carry `Vec::push` capability through helper calls without exposing `&mut Vec<_>` parameters. Leaf helpers return `WidgetNode { … }` directly, while only parents bind a mutable node for child insertion. `ClickArea` construction is mutable only when an `onClicked` handler is lowered. These are emitted-code lint conformance changes; tuple destructuring and QML tag values remain source-compatible. `QT_EMIT_VERSION_RLVGL` advances `22 → 23`. |
