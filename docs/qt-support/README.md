...
| `QT-05g` | [`05g-state-predicate-bindings.md`](./05g-state-predicate-bindings.md) — rlvgl emit: state-predicate → Image-source bindings (`is_active`); istate linkage v2 | normative — ratified; implementation shipped (Play/Pause live on ESP32-P4) |
| `QT-05h` | [`05h-visibility-bindings.md`](./05h-visibility-bindings.md) — rlvgl emit: state-predicate → widget visibility (`visible: <ctx>.<state>`); mute region remodel | normative — ratified; implementation shipped (mute visibility live on ESP32-P4) |
| `QT-05i` | [`05i-chained-predicate-bindings.md`](./05i-chained-predicate-bindings.md) — rlvgl emit: chained state-predicate → Image-source bindings (repeat-mode icon); shuffle/repeat region remodel | normative — ratified; implementation shipped (repeat live on ESP32-P4) |
| `QT-05j` | [`05j-button-event-bindings.md`](./05j-button-event-bindings.md) — rlvgl emit: `submitBtnSetupEvent("…")` → emitted tap-target table (shuffle tap wired; consumer owns the QML→machine map) | normative — ratified; implementation shipped (host-verified) |
| `QT-05k` | [`05k-external-text-bindings.md`](./05k-external-text-bindings.md) — rlvgl emit: `text: <obj>.<prop>` external-object Label text → `Binding::ExternalText` + `apply_external_text` (consumer owns the key→value resolver) | normative — ratified; implementation shipped (host-verified; P4 caption pending bench round) |
...
| `QT-10`  | [`10-release.md`](./10-release.md) — strict-mode acceptance + release tag (auto-mode half) | normative — ratified; strict-mode-2 shipped (release tag is user-driven) |
...
| 2026-07-12 | **QT-10 strict-mode-2 amendment shipped** ([`10-release.md`](./10-release.md)). Reconciles the live 29-chapter set, advances `QT_EMIT_VERSION_RLVGL` to 23 and `QT_FAMILY_STRICT_VERSION` to 2, and pins the new generated-Rust contracts: precedence-aware bounds expressions, snake-case Rust identifiers with original QML tags, private binding sinks, direct leaf returns, conditional widget mutability, and the named `BuiltScreen` result alias. |
...
| 2026-06-28 | **QT-05k ratified + shipped.** Closes the last reactive surface on the Bolero media frame: the source caption `text: audioPlayer.currentPlayUrlFileName` (an external Qt context object, not machine state). New `Binding::ExternalText(ExternalTextBinding { label, key })` (carries an `Rc<RefCell<Label>>` handle — the `Widget` trait has no downcast), an app-agnostic `pub const EXTERNAL_TEXT_BINDINGS: &[(&str, &str)]` = `(node tag, verbatim external key)` (rustfmt-skipped), and a consumer entry point `pub fn apply_external_text(bindings, resolve)`. New emitter pieces: `parse_external_text_ref` (pure dotted-identifier path, ≥2 segments, head ≠ scxml context), a Label `text:` sub-branch (after literal/`sm.dm`/`ScreenState`), `emit_external_text_binding_struct`/`emit_apply_external_text_fn`, `RlvglEmitCtx.{used_external_text, external_text_bindings}`; `refresh_bindings` gains a **no-op** `Binding::ExternalText(_) => {}` arm (exhaustiveness — external text is resolver-driven, not machine-driven). The QML expression round-trips verbatim (**derive**); the key→value resolver is **consumer-owned** (the role Bolero's C++ `audioPlayer` plays), NOT emitted — mirrors QT-05j's `MEDIA_FUNC_MAP`. `media_player_gen.rs` regenerated via the emitter: `textSource` gains a `Binding::ExternalText`; `EXTERNAL_TEXT_BINDINGS` lists `("textSource", "audioPlayer.currentPlayUrlFileName")`. `MediaPlayerSkin` adds a skin-owned `EXTERNAL_TEXT_SOURCES` resolver + calls `apply_external_text` at construction; new host gate asserts the caption shows the consumer-resolved value. v1 scope: linkage-v2, pure dotted external-property paths (JS-function text, timeline, no-SM external text deferred). `QT_EMIT_VERSION_RLVGL` 21→22; goldens regenerated (version line only); strict-mode chapter set → 29 + pin → 22. P4 on-panel caption pending an authorized bench round. ([`05k-external-text-bindings.md`](./05k-external-text-bindings.md)) |
| 2026-06-28 | **QT-05j ratified + shipped.** Closes the view→machine half for buttons: lowers `<ctx>.submitBtnSetupEvent("MediaFunc.X")` handlers (and the Repeater delegate's `submitBtnSetupEvent(eventName, …)` via each model entry's `eventName`) into a new emitted `pub const BUTTON_TAP_EVENTS: &[(&str, &str)]` = `(node tag, raw QML "MediaFunc.*" event)`. New emitter pieces: `parse_submit_btn_event`, `extract_model_event_name`, `synthesize_button_tap_tags` (gives an untagged `submitBtnSetupEvent` button a deterministic `__btn_<event>` tag), `RlvglEmitCtx.button_tap_events`. The QML event round-trips verbatim (**derive**); the `MediaFunc.* → Inp.Media.*` map is **consumer-owned** (the role Bolero's C++ glue plays), NOT emitted — the general emitter stays app-agnostic. `media_player_gen.rs`: the shuffle button gains tag `__btn_mediafunc_shuffle` (was untagged → inert on-panel); `BUTTON_TAP_EVENTS` lists transport + repeat + shuffle (+ unmodelled scan, ignored). `MediaPlayerSkin` retires QT-05i's hand-written `TAP_CONTROLS` for `BUTTON_TAP_EVENTS` ⋈ a skin-owned `MEDIA_FUNC_MAP`; new pixel gate asserts a **tap** on shuffle swaps its icon. **Amends QT-05g §9** (button-event lowering): map ownership specified (consumer), handler lowered to a tap *table* not in-tree `machine.step` — the structural "no in-tree dispatch" property holds. `QT_EMIT_VERSION_RLVGL` 20→21; goldens regenerated (version line only); strict-mode chapter set → 28 + pin → 21. ([`05j-button-event-bindings.md`](./05j-button-event-bindings.md)) |
| 2026-06-28 | **QT-05i ratified + shipped.** Closes the chained state-predicate → Image-source loop the repeat-mode icon needs (the form QT-05g §5 reserved). New `Binding::Chain(PredicateChainBinding)` variant + `PredicateChainBinding { image, arms: Vec<PredicateArm>, default: ImageArt }` / `PredicateArm { state_id, art }`: a chained `source: <ctx>.<s1> ? "A" : <ctx>.<s2> ? "B" : "C"` (≥2 arms + literal else) lowers to a first-active-wins `is_active` ladder (else the resting default) under a `// QT-05i predicate-chain-bound:` marker via a new `qt_predicate_chain_image` helper + `parse_chained_predicate_source` walker (runs after QT-05g's binary `parse_predicate_source`). Executes the shuffle/repeat half of PCDN-05g-1: `media_player_normalized.scxml`'s `transportActive` `<parallel>` (from QT-05h) gains a shuffle region (`mediaPlayMixModeOff`/`On`, `Inp.Media.Shuffle`) and a repeat region (`mediaRepeatOff`/`Track`/`Folder`, `Inp.Media.Repeat` none→track→folder→none keeping `s_repeat` in sync) — NO cross-region `In()` guard. `media_player_gen.rs` regenerated: the repeat button is now a `Binding::Chain` over `mediaRepeatTrack`/`mediaRepeatFolder` (default NoRepeat; all three repeat assets now display, previously only TrackRepeat statically) and the shuffle button's QT-05g `Binding::Predicate` is now backed. Skin generalises tap routing to a `TAP_CONTROLS` table (adds `repeatBtn`→`Inp.Media.Repeat`); pixel gates assert the repeat icon cycles Off→Track→Folder→Off and shuffle toggles. Real shuffle-tap wiring deferred (the `MediaShuffleButton` is untagged upstream). `QT_EMIT_VERSION_RLVGL` 19→20; all rlvgl goldens regenerated (version line only); QT-10 strict-mode chapter set → 27 + source pin → 20. ([`05i-chained-predicate-bindings.md`](./05i-chained-predicate-bindings.md)) |
| 2026-06-27 | **QT-05h ratified (concepts).** Adds the state-predicate → **visibility** binding: `visible: <ctx>.<state>` on an `Image` (e.g. `visible: scxmlBolero.muteOn`) lowers to a new `Binding::Visibility(VisibilityBinding)` whose `refresh` calls `Image::set_hidden(!machine.is_active("<state>"))` (a hidden Image's `draw` is a no-op). Rides QT-05g's linkage v2 + `--scxml-context` — no §6 amendment. Executes ratified PCDN-05g-1: `media_player_normalized.scxml` re-models the transport playback states into a `<parallel>` alongside a real mute region (`muteOff`/`muteOn`, toggled by `Inp.Media.Mute`), so `is_active("muteOn")` is the predicate primitive — NO cross-region `In()` guard (transport `cond="!s_mute"` guards keep the var, kept in sync by the region). Fixes the static gen always-painting `imgMute`; it's now hidden at rest. Scope = mute icon; shuffle/repeat regions + chained-predicate `source:` deferred. `QT_EMIT_VERSION_RLVGL` 18→19 lands in implementation commits ([`05h-visibility-bindings.md`](./05h-visibility-bindings.md)). |
