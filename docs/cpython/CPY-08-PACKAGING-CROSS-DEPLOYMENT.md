...
**Status:** Draft 2026-08-18. Six packaging-policy PCDNs resolved through
2026-08-19; build, target import, and reproducibility evidence remain open. Not
ratified.
...
- publication and installation through PyPI;
...
| **Source Distribution** | PyPI/PEP 517 source artifact containing the CPython binding and every selected local rlvgl Rust source needed to build it without a repository checkout or VCS fetch. | Owned by CPY-08; built by pinned maturin. |
...
### 5.1 Self-contained native source build

The PyPI source distribution MUST carry the `rlvgl-cpython` crate and every
selected workspace path dependency needed to build the pinned rlvgl graph. Its
manifests MUST resolve entirely within the extracted source tree or to exact
registry packages recorded by the retained lock/dependency inventory. A local
path outside the archive, mutable Git/VCS dependency, implicit repository
submodule, or build-time source checkout is forbidden.

Building the sdist through standard PEP 517 isolation invokes pinned maturin,
Cargo, and the selected CPython toolchain to compile `rlvgl._native`. The sdist
includes the exact Cargo manifests/lock authority, descriptor-generated Python
files, licenses/notices, and a source manifest hashing the bundled rlvgl files.
Release evidence additionally retains a vendored/offline dependency input so
the same native graph can be rebuilt without network access.

Prebuilt wheels contain the compiled rlvgl graph and perform no compiler,
download, or source acquisition at install, import, or runtime. Backend support
is declared per wheel in the Artifact Manifest. A general-host Linux wheel may
claim both headless and WLD-backed profiles only after both are closed and all
WLD/system dependencies are declared; target-specific embedded bundles remain
separate artifact rows.
...
1. one self-contained `rlvgl` source distribution plus retained
   version-specific host wheels for each claimed CPython/host row;
2. a PyPI/TestPyPI metadata and install-proof row for the source distribution
   and every public general-host wheel;
3. a target-specific embedded Direct Deployment wheel plus offline Rootfs
   Bundle for Debian 13 `armhf` and the separate `arm64` import/runtime row;
4. generated typing and descriptor-fingerprint artifacts inside each wheel;
5. checksummed Artifact/Baseline Manifests, licenses, and dependency inventory.

All artifacts are retained in the project's build/evidence store. The general-
host `rlvgl` source distribution and supported wheels MUST also be publishable
to PyPI with accepted project metadata; a TestPyPI upload/install smoke proof
is part of CPY-08 evidence. Production PyPI upload is still a separate
owner-authorized CPY-09 release action. A source distribution is required for
native source builds but never substitutes for a target wheel or Import Proof.
Embedded Rootfs Bundles and nonstandard target artifacts need not be uploaded
to PyPI merely because the general-host distribution is publishable there.
...
| **INV-CPY-08-9** | The PyPI sdist MUST rebuild the pinned native rlvgl graph without a repository checkout, external path dependency, or mutable VCS fetch; wheels MUST install/import without compiling or downloading native code. | Sdist-content, isolated-build, wheel-install, and source-manifest audit |
...
| Cargo packages | Follow CRATES-CI publication order and dry-run gates; the PyPI sdist still carries every selected local path dependency needed for an isolated native build. |
...
- Publishing to public indexes other than the required PyPI channel in the
  first general-host release.
...
- **PCDN-CPY-08-003 — Distribution channels — Accepted as amended
  2026-08-19.** Retain the sdist, wheels, Rootfs Bundles, and manifests in the
  project evidence/artifact store for offline installation. The general-host
  `rlvgl` sdist and supported wheels MUST be PyPI-publishable and pass TestPyPI
  upload/install evidence; production PyPI upload remains a separate
  owner-authorized CPY-09 release action and requires verified control of the
  normalized PyPI project name at release time.
...
- [ ] Offline installation, checksums, licenses, and dependency/SBOM records are
      complete.
- [ ] The self-contained sdist rebuilds the pinned rlvgl graph in isolation,
      and TestPyPI wheel/sdist installation succeeds without repository paths.
- [ ] The release owner verifies control of the normalized PyPI project name;
      drafting-time name availability is not treated as a reservation.
...
upgrade/offline-install plus TestPyPI tests. Ratification would unblock packaging
...
### 0.3.0 — 2026-08-19 — require a PyPI-publishable native distribution

**Author:** Ira Abbott / OpenAI Codex

**Change kind:** semantic

**Touches:** INV-CPY-08-1, INV-CPY-08-2, INV-CPY-08-7, INV-CPY-08-9,
PCDN-CPY-08-003, §1, §3, §5, §8, §9, §10, §11, §12, §14

**Commits:** pending

**Summary:** Makes PyPI a required supported channel and requires a
self-contained source distribution that carries and builds the pinned native
rlvgl graph alongside truthful prebuilt wheels.

#### Rationale

Users should be able to install the public `rlvgl` package normally or build it
from published source without first cloning this monorepo and reconstructing
workspace paths. A retained source manifest, exact Cargo authority, isolated
maturin build, and TestPyPI proof preserve provenance while wheels keep the
ordinary install path compiler-free.

Considered and rejected: wheel-only publication, an sdist that references the
developer checkout, mutable Git dependencies, downloading rlvgl during import,
or treating TestPyPI success as authorization for a production release.

What deliberately did not change: actual production upload remains an explicit
owner/CPY-09 action; embedded Rootfs Bundles retain their separate target rows;
ABI, WLD feature, and system-library claims still require their own evidence.
