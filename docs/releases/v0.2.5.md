...
state-chart-to-reactive-UI workflow using public browser and MCP iState tools.
The companion [`docs/ratatui-tutorial/`](../ratatui-tutorial/README.md) builds
the retained-cell bridge, hybrid window, host verification, and bare-metal
proof as a separate focused series.
...
- **[`docs/sctd-tutorial/`](../sctd-tutorial/README.md)** — a new
  ...
  desktop, ESP32-P4, and STM32H747I-DISCO. It documents browser and MCP iState
  generation and is written for readers outside the monorepo.
- **[`docs/ratatui-tutorial/`](../ratatui-tutorial/README.md)** — a separate
  five-chapter guide to `RlvglBackend`, the retained cell surface,
  `RatatuiView`, hybrid native/Ratatui composition, live snapshots, host-first
  verification, and the Rust-only STM32H747I-DISCO path.
...
chunks in the image decoders, host-simulator pixel loops, SCTD asset paths, and
Qt-emitted image helpers, plus native slice fill for Canvas. This satisfies the
refreshed nightly Clippy gate while preserving existing pixel-buffer behavior.
