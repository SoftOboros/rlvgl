...
| 2026-05-04 | IMPLEMENTATION | APP-05 family complete (a/b/c/d/e + f). Sub-letter analysis [`APP-05-A.md`](APP-05-A.md) committed Option 1 (in-binary Rust tables). New module `src/bin/creator/app/feature_graphs.rs` (~410 lines) with `ProngTemplate` shape (`base_deps`, `target_cfg_deps`, `build_deps`, `feature_expansions`, `default_features` policy, `bin_required_features`) and `lookup(prong, generator, vendor, board) -> Option<&ProngTemplate>`. `emit_cargo_toml` (chapter 02 §8 preamble's frozen "per-prong template owns the graph" rule) rewritten to consult the lookup; manifest features expand to template entries when the tuple matches, fall through to `feat = []` placeholder when no template registered. Five templates shipped (one per round-trip target tuple): APP-05a `BBB_LINUX` (linux + hand_written + ti + beaglebone_black_nhd_cape), APP-05b `BEETLE_ESP_HAL` (bare_metal + hosted + esp + beetle_esp32c3 esp_hal half), APP-05c `BEETLE_BSP_PAC` (bare_metal + creator-bsp-pac + esp + beetle_esp32c3 bsp_pac half), APP-05d `H747_FREERTOS` (freertos + hand_written + stm + stm32h747i_disco; covers both `app.yaml` + `app-with-sm.yaml`), APP-05e `H747_ZEPHYR` (zephyr + hand_written + stm + stm32h747i_disco). H747 base deps + cross-compile deps + cm7 expansion factored to shared statics so APP-05d/e use one source. APP-05f discipline scanner: meta-tests assert every committed manifest's tuple resolves to a template AND every manifest feature appears in that template's `feature_expansions` table; aggregate no-`TODO(template-tuning)` check across all six committed manifests. New dev-dep `toml = "0.8"` for parity-test parsing of multi-line `[features]` lists. New test file `tests/creator_app_feature_graph.rs` with 19 tests (3 per phase a/b/c/d/e + 4 cross-cutting f). 19/19 pass; full creator-feature integration suite remains green (no regressions). The §11 v1 non-goal "compile-verification of the emitted Cargo.toml" still applies — the parity gates check structural shape (set-equal feature expansions, subset deps), not buildability. v0 closure of the §8 preamble's feature-graph rule. |
