...
use stm32mp1::stm32mp1257 as pac;
...
pub fn configure_pins_pac(dp: &pac::Peripherals) {
    // GPIOZ
    dp.GPIOZ.pupdr.modify(|r, w| unsafe {
        let mut bits = r.bits();
        let shift = 0 * 2;
        bits &= !(0b11 << shift);
        bits |= 0b00 << shift;
    ...
    dp.GPIOZ.otyper.modify(|r, w| unsafe {
        let mut bits = r.bits();
        bits &= !(1 << 0);
        bits &= !(1 << 1);
        bits &= !(1 << 2);
    ...
    dp.GPIOZ.ospeedr.modify(|r, w| unsafe {
        let mut bits = r.bits();
        let shift = 0 * 2;
        bits &= !(0b11 << shift);
        bits |= 0b11 << shift;
    ...
    dp.GPIOZ.afrl.modify(|r, w| unsafe {
        let mut bits = r.bits();
        w.bits(bits)
    });
    dp.GPIOZ.afrh.modify(|r, w| unsafe {
        let mut bits = r.bits();
        let afr_shift = (0 % 8) * 4;
        bits &= !(0xF << afr_shift);
