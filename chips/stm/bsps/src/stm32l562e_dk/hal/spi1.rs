...
use stm32l5xx_hal::{gpio::Speed, gpiog::*, pac, prelude::*};
...
pub fn configure_pins_hal(dp: &pac::Peripherals) {
    ...
    // PG2 SPI1_SCK AF0
    let pg2 = gpiog.pg2.into_alternate::<0>().set_speed(Speed::VeryHigh);

    // PG3 SPI1_MISO AF0
    let pg3 = gpiog.pg3.into_alternate::<0>().set_speed(Speed::VeryHigh);

    // PG4 SPI1_MOSI AF0
    let pg4 = gpiog.pg4.into_alternate::<0>().set_speed(Speed::VeryHigh);

    // PG5 SPI1_NSS AF0
    let pg5 = gpiog.pg5.into_alternate::<0>().set_speed(Speed::VeryHigh);
}
