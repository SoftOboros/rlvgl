...
pub fn init() {
    ...
    p.GPIO
        .func_out_sel_cfg(1)
        .modify(|_, w| unsafe { w.out_sel().bits(46) });

    p.GPIO
        .func_in_sel_cfg(46)
        .modify(|_, w| unsafe { w.in_sel().bits(1).in_inv_sel().clear_bit() });
    ...
    p.GPIO
        .func_out_sel_cfg(2)
        .modify(|_, w| unsafe { w.out_sel().bits(45) });

    p.GPIO
        .func_in_sel_cfg(45)
        .modify(|_, w| unsafe { w.in_sel().bits(2).in_inv_sel().clear_bit() });
