...
//! PAC-style BSP initialization for Board.
//!
...
fn init_pac(dp: pac::Peripherals) {
    ...
    dp.RCC.ahb4enr.modify(|_, w| w.gpioeen().set_bit());
    dp.RCC.ahb4enr.modify(|_, w| w.gpiofen().set_bit());
    dp.RCC.ahb4enr.modify(|_, w| w.gpiogen().set_bit());
    dp.RCC.ahb4enr.modify(|_, w| w.gpiohen().set_bit());
    dp.RCC.ahb4enr.modify(|_, w| w.gpioien().set_bit());
    dp.RCC.ahb4enr.modify(|_, w| w.gpiojen().set_bit());
    dp.RCC.ahb4enr.modify(|_, w| w.gpioken().set_bit());
// Configure pins
    // PA1 ETH_REF_CLK AF0
    let shift = 1 * 2;
    let moder_bits = 0b10;
