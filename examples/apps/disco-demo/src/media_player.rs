...
//!
//! # Runtime
//! This vendored module uses `alloc` collections so it can run inside the
//! `no_std` disco demo app crate.
//!
//! # Blocked constructs
...
impl Machine {
    ...
    pub fn step(&mut self, event_name: &str, event_data: Value) {
        ...
        // tokens are registered with / canceled from the timer queue; others are
        // processed normally.
        let mut safety = 0usize;
...
///
/// Leaks one allocation for the machine IR's static lifetime.
...
fn build_machine_ir() -> MachineIrData {
    MachineIrData {
        ...
        datamodel: vec![
            DataSlotIr {
                id: "s_source".to_string(),
                initial_value: Some(Value::Str("USB".to_string())),
            },
            DataSlotIr {
                id: "s_mute".to_string(),
            ...
            DataSlotIr {
                id: "s_repeat".to_string(),
                initial_value: Some(Value::Str("none".to_string())),
            },
            DataSlotIr {
                id: "i_player_state".to_string(),
                initial_value: Some(Value::Int(0_i64)),
            },
        ],
        children: vec![
            StateNodeIr::State {
                                                    ...
                                                    branches: vec![
                                                        IfBranchIr {
                                                            guard: Some(ExprIr::BinOp(
                                                                "==".to_string(),
                                                                Box::new(ExprIr::VarRead(
                                                                    "s_repeat".to_string(),
                                                                )),
                                                                Box::new(ExprIr::StringLiteral(
                                                                    "track".to_string(),
                                                                )),
                                                            )),
                                                            body: vec![
