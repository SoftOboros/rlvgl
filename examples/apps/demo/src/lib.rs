...
const JPEG_LOGO: &[u8] = &[
    ...
    0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32,
    0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0xff, 0xc0, 0x00, 0x11, 0x08,
    ...
    0x03, 0x01, 0x00, 0x02, 0x11, 0x03, 0x11, 0x00, 0x3f, 0x00, 0xe2, 0xe8, 0xa2, 0x8a, 0xf9,
    0x93, 0xf7, 0x13, 0xff, 0xd9,
];

#[cfg(feature = "gif")]
const GIF_LOGO: &[u8] = &[
    0x47, 0x49, 0x46, 0x38, 0x37, 0x61, 0x01, 0x00, 0x01, 0x00, 0xf0, 0x00, 0x00, 0xff, 0x00,
...
impl Application for DemoApp {
    ...
    fn build(&mut self, width: u32, height: u32) -> WidgetNode {
        ...
        let jpeg_demo: WidgetSlot = Rc::new(RefCell::new(None));
        {
            ...
            plugins
                .borrow_mut()
                .set_on_click(move |_btn: &mut Button| {
                    if let Some(menu_w) = menu_ref.borrow_mut().take() {
                        pending_rm.borrow_mut().push(menu_w);
                    } else {
                        let menu_w: WidgetHandle =
                            Rc::new(RefCell::new(Container::new(Rect {
                                x: 10,
                                y: 70,
                                width: 100,
                                height: 170,
...
pub fn build_plugin_demo(root_w: u32, root_h: u32) -> WidgetNode {
    let (pixels_vec, width, _) =
        qrcode::generate(b"https://github.com/SoftOboros/rlvgl").unwrap();
