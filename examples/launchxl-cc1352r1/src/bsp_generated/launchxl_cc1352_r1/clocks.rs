//! Clock tree initialization for LAUNCHXL-CC1352R1.
//!
//! Peripheral clock gating and reset on CC1352R are controlled
...
/// peripheral's `prcm:` entry in `chipdb/rlvgl-chips-ti/db/chips/cc1352_r.yaml`
/// names the clock-gate register/field and the reset register/field;
/// fields are written via the svd2rust-shaped `cc13x2_26x2_pac` PAC.
pub fn init() {
    let p = unsafe { pac::Peripherals::steal() };
    
    
    
    // uart0 — clock enable clk_en.uart0 on prcm.uartclkgr,
    //             reset uart0 on prcm.resetuart
    // `clk_en` is a multi-bit enum FieldWriter in this PAC
    // vintage; select the instance variant rather than calling `.set_bit()`.
    p.prcm.uartclkgr().modify(|_, w| w.clk_en().uart0());
    ...
    p.prcm.resetuart().modify(|_, w| w.uart0().clear_bit());
    
    
    
    
    // i2c0 — clock enable clk_en on prcm.i2cclkgr,
    //             reset i2c on prcm.reseti2c
    p.prcm.i2cclkgr().modify(|_, w| w.clk_en().set_bit());
    // Stage the gate change: pulse CLKLOADCTL.LOAD and wait for LOAD_DONE.
    p.prcm.clkloadctl().modify(|_, w| w.load().set_bit());
    while p.prcm.clkloadctl().read().load_done().bit_is_clear() {}
    // Release the peripheral from reset (clear bit deasserts reset).
    p.prcm.reseti2c().modify(|_, w| w.i2c().clear_bit());
