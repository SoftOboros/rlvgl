//! Compile-verify the ESP32-H2 BSP generator output against the real
//! `esp32h2` PAC crate on `riscv32imac-unknown-none-elf`.
//!
...
[package]
name = "bsp-esp32h2-compile-verify"
version = "0.0.0"
edition = "2021"
publish = false
...
[dependencies]
esp32h2 = { version = "0.19", features = ["critical-section", "rt"] }
"#;
...
fn compile_verify_board(board_slug: &str, rendered_subdir: &str, tag: &str) {
    ...
    let chip = load_chip_db("esp32h2").expect("chip yaml");
    ...
    if !output.status.success() {
        ...
        panic!(
            "generator output for {board_slug} failed to compile against esp32h2 0.19 on {target}"
        );
    }
}
...
//
// Same divergence shape as C6: 3 × E0433 (could not find `Peripherals` in
// `pac`) in clocks.rs/io_mux.rs/peripherals.rs at `pac::Peripherals::steal()`,
// cascading into 13 × E0282 (type annotations needed) on the
// `.modify(|_, w| ...)` writer closures. PAC vintage divergence from
// `esp32c3 = 0.31`; not a chipdb issue. Most likely responsible:
// `pac.rs.jinja` (re-export shape) and/or `peripherals.rs.jinja`
// + sibling templates.
#[test]
#[ignore = "scaffolded, FAILING — pac::Peripherals reexport + writer closure inference do not match esp32h2 0.19; follow-up CHIPS-ESP-NN"]
