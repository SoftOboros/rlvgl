...
//
// Generated by `rlvgl-creator qt emit --target rlvgl` from `tests/fixtures/qt/centered.qml`.
...
pub const QT_SOURCE: &str = "tests/fixtures/qt/centered.qml";
...
fn build_root(
    ...
    // QT-03c centered: anchors.centerIn: parent (child 50×50)
    ...
    node.children.push(build_badge(child_bounds, Rc::clone(&state), label_bindings));
    node
}

// QML type: `Rectangle` (id: `badge`)
#[rustfmt::skip]
fn build_badge(
    bounds: Rect,
    state: Rc<RefCell<ScreenState>>,
    ...
    w.style.bg_color = Color(0xff, 0x88, 0x00, 0xff);
