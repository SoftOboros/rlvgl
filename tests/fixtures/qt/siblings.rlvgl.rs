...
//
// Generated by `rlvgl-creator qt emit --target rlvgl` from `tests/fixtures/qt/siblings.qml`.
...
pub const QT_SOURCE: &str = "tests/fixtures/qt/siblings.qml";
...
pub fn build_screen(
    ...
    let node = build_node_0(bounds, Rc::clone(&state), &mut label_bindings);
...
fn build_node_0(
    ...
    let cb_2 = Rect {
        x: bounds.x,
        y: cb_1.y,
        width: ((cb_1.x) - (bounds.x)),
        height: (((cb_1.y + cb_1.height)) - (cb_1.y)),
    };
    let cb_3 = Rect {
        x: bounds.x,
    ...
    node.children.push(build_header(cb_0, Rc::clone(&state), label_bindings));
    node.children.push(build_art(cb_1, Rc::clone(&state), label_bindings));
    node.children.push(build_title(cb_2, Rc::clone(&state), label_bindings));
    node.children.push(build_footer(cb_3, Rc::clone(&state), label_bindings));
    node
}

// QML type: `Rectangle` (id: `header`)
#[rustfmt::skip]
fn build_header(
    bounds: Rect,
    state: Rc<RefCell<ScreenState>>,
