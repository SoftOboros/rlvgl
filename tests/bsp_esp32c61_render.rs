//! Snapshot tests for the ESP32-C61 BSP render pipeline.
//!
//! Drives [`render_esp_pac`] end-to-end against the minimal ESP32-C61
//! board spec embedded in `rlvgl-chips-esp`, writing output to a tempdir
//! and snapshotting the linker-script outputs (memory.x, esp32_c61.x).
...
fn render_to_tempdir() -> (tempfile::TempDir, std::path::PathBuf) {
    let chip = load_chip_db("esp32c61").expect("esp32c61 chip yaml");
    let board = load_board_db("esp32c61_minimal").expect("esp32c61_minimal board yaml");
    let ir = merge(chip, board).expect("merge ok");
    let tmp = tempfile::tempdir().expect("tempdir");
    let written = render_esp_pac(&ir, tmp.path()).expect("render ok");
    // 6 Rust files + memory.x + esp32_c61.x.
    assert_eq!(written.len(), 8);
    let bsp_dir = tmp.path().join("esp32_c61_minimal");
...
fn produces_expected_file_set() {
    ...
    for name in ["mod.rs", "memory.x", "esp32_c61.x"] {
...
fn snapshot_memory_x() {
    ...
    insta::assert_snapshot!("esp32c61_minimal__memory_x", text);
}

#[test]
fn snapshot_chip_x() {
    let (_tmp, bsp_dir) = render_to_tempdir();
    let text = fs::read_to_string(bsp_dir.join("esp32_c61.x")).expect("read esp32_c61.x");
    insta::assert_snapshot!("esp32c61_minimal__esp32_c61_x", text);
}
