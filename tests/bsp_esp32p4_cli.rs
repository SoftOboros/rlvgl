//! End-to-end CLI test for `rlvgl-creator bsp from-yaml` with ESP32-P4.
//!
//! Invokes the compiled `rlvgl-creator` binary against the DFR1172 FireBeetle
//! 2 P4 chipdb spec and asserts that the six expected files are produced.
#![cfg(feature = "creator")]
...
#[test]
fn from_yaml_esp_generates_beetle_esp32p4_bsp() {
    ...
    let bsp_dir = out.join("dfr1172_fire_beetle_2_p4");
    assert!(
        bsp_dir.is_dir(),
        "expected bsp dir at {}",
        bsp_dir.display()
