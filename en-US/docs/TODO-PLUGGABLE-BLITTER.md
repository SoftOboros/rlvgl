...
| [x] | Implement M2M blend (FG over BG, const/per-pixel alpha) | PAC | Straight-alpha assumption; document it. |
...
| Done | Description | Dependencies | Notes |
|---|---|---|---|
| [x] | Integrate `fontdue` text rasterizer into `BlitterRenderer` | `fontdue` | Cache glyphs as `Surface`s; support CPU/WGPU/DMA2D paths. |
