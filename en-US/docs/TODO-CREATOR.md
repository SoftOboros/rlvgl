...
_A single markdown document that structures the work as one **Epic** with sectioned user-story tables. Each section begins with a brief description (user story) and a checklist table._

---

## Epic Overview
**Epic:** Build **rlvgl-creator**, a UI + CLI tool that imports, normalizes, previews, and vendors assets for rlvgl projects while scaffolding dual-mode asset crates and minimizing footprint on `no_std + alloc` targets.

**Outcomes:**
- Repeatable pipelines for raw RGBA image sequences, fonts, and Lottie.
- Strict naming/path policies with automatic fix guidance.
...
_User story: As a maintainer, I want guardrails so teams can safely scale assets without drift._
...
_User story: As a user, I can consume assets by embedding bytes or vendoring files without runtime dependencies._
...
| [x] | Generate `src/lib.rs` — vendor: `vendor_api::{copy_all, generate_rust_module}`. | std fs, tera | `$OUT_DIR` safe paths.
...
_User story: As a stakeholder, I can quickly verify value with a working vertical slice._
...
_User story: As a power user, I can further optimize pipelines and packaging._
...
_User story: As a newcomer, I can become productive with clear examples and guides._
