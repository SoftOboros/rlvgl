...
## Current gap
- The current importer only emits pin → signal → AF maps.
...
2. [ ] **Lookup normalization** – Centralize maps translating Cube strings to MODER/OTYPER/OSPEEDR/PUPDR bits and HAL enum names. Embed derived bitfields and HAL strings into each pin context. Pins missing an AF store `null` so templates can skip AFR writes.
...
4. [ ] **HAL template rules** – Generate `into_alternate`, `into_push_pull_output`, etc., using normalized speed/pull/otype helpers.
...
## Lookup tables
Normalized maps translate Cube strings into register bits and HAL enum names:
